...
| A credential file readable by another account on the host | `owner-only` is the default and refuses every group/other bit before the token or App key is used. A deployment may name a wider `filePermissions` policy for what its platform creates; every policy above the default is warned by doctor and at startup. The two middle policies require controller ownership only when widened group/other bits are present, which preserves the compatibility and monotonicity of operator-owned `0600` files. `ignore-mode-and-owner` explicitly delegates those POSIX decisions, but no policy accepts a non-regular or larger-than-1-MiB input. The opened descriptor is inspected and then read, so a path replacement cannot substitute unverified bytes between those operations | Table-tested: every adjacent rung is monotonic over all group/other bits and both ownership states; each rung exhaustively accepts exactly the bits it names on a controller-owned file; foreign ownership is refused only when a middle rung relies on widened bits; devices and oversized files are refused under `ignore-mode-and-owner`; a refusal names the narrowest accepting rung |
