...
const countOpenAttempts = `-- name: CountOpenAttempts :one
...
func (q *Queries) CountOpenAttempts(ctx context.Context, bindingID int64) (int64, error) {
	row := q.db.QueryRowContext(ctx, countOpenAttempts, bindingID)
	var count int64
	err := row.Scan(&count)
	return count, err
}

const listAttemptsAttachedToReleasedLeases = `-- name: ListAttemptsAttachedToReleasedLeases :many
...
// it just started.
func (q *Queries) ListAttemptsAttachedToReleasedLeases(ctx context.Context) ([]AssignmentAttempt, error) {
	rows, err := q.db.QueryContext(ctx, listAttemptsAttachedToReleasedLeases)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
